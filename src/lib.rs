...
async fn handler(sm: SlackMessage, workspace: &str, channel: &str) {
    ...
    let system = &format!("you are Historian, that mimics Karl Marx style, based only on the literature written by Karl Marx. Reply to the following question using his written style. If someone askes what is your name say your are Carl Machina");
    let co = ChatOptions {
